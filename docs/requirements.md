...
| FR-029 | Hauptlisten nur mit Gemüse       | Als Administrator/in möchte ich Hauptliste, Depot- und Mengenübersicht samt Einheiten-Total auf das Produkt Gemüse beschränkt haben, damit die Übersichten durch die Hofprodukte nicht unlesbar werden.               | UC-004 | Hoch      | Implementiert |
...
| FR-052 | Bezüger/innen alphabetisch sortiert | Als Depotverantwortliche/r möchte ich die Bezüger/innen je Depot alphabetisch aufgelistet haben, damit ich beim Verteilen einen einzelnen Namen finde, ohne die Seite abzusuchen.                | UC-004 | Hoch      | Implementiert |
| FR-053 | Nur aktive Bestandteile auf Produktlisten | Als Depotverantwortliche/r möchte ich auf einer Produktliste nur Abos sehen, deren Bestandteil dieses Produkts am Stichtag aktiv ist (auch wenn bereits gekündigt), damit keine leeren Zeilen für bloss bestellte oder bereits deaktivierte Bestandteile erscheinen. | UC-004 | Hoch      | Implementiert |
| FR-054 | Menge je Produktgrösse           | Als Depotverantwortliche/r möchte ich je Produktgrösse genau die Anzahl aktiver Bestandteile dieser Grösse sehen, damit z.B. bei 2× Weizenmehl Halbweiss 3kg, 1× Weizenmehl Ruch 1kg und 1× Dinkelmehl Halbweiss 1kg nicht in jeder Mehl-Spalte eine 4 steht. | UC-004 | Hoch      | Implementiert |
| FR-055 | Grösse je Abo-Typ zuordnen       | Als Administrator/in möchte ich beim Abo-Typ festlegen, welche Produktgrösse er auf den Depotlisten bezieht, damit neue oder umbenannte Mehl- und Käsesorten ohne Programmänderung richtig gezählt werden und das Bestellformular seine Gliederung nach Abo-Paketen behält. | UC-004 | Hoch      | Implementiert |
